...
export class InventoryService {
  ...
  private updateItemFreshness(item: Item, dt: number): void {
    ...
    if (newCondition === ItemCondition.SPOILED) {
      this.handleFoodSpoilage(item);
    }
  }

  private calculateConditionFromFreshness(freshnessPercent: number): ItemCondition {
    if (freshnessPercent === 0) return ItemCondition.SPOILED;
    if (freshnessPercent < 0.2) return ItemCondition.DAMAGED;
    if (freshnessPercent < 0.5) return ItemCondition.WORN;
    if (freshnessPercent < 0.8) return ItemCondition.GOOD;
  ...
  damageItem(slotId: string, damage: number): boolean {
    ...
    if (newCondition === ItemCondition.BROKEN || newCondition === ItemCondition.DESTROYED) {
  ...
  private calculateConditionFromDurability(durabilityPercent: number): ItemCondition {
    if (durabilityPercent === 0) return ItemCondition.DESTROYED;
    if (durabilityPercent < 0.25) return ItemCondition.BROKEN;
    if (durabilityPercent < 0.5) return ItemCondition.DAMAGED;
    if (durabilityPercent < 0.7) return ItemCondition.WORN;
    if (durabilityPercent < 0.9) return ItemCondition.GOOD;
    return ItemCondition.PRISTINE;
  }
  ...
  private updateDurabilityBasedCondition(item: Item, newCondition: ItemCondition): void {
    // This method would update the item's condition based on external factors
    // For now, it's a placeholder for future enhancements
  }

  private updatePerishableCondition(item: Item, newCondition: ItemCondition): void {
    // This method would update the item's condition based on external factors
    // For now, it's a placeholder for future enhancements
  }
  ...
  getBrokenItems(): InventorySlot[] {
    return this.inventory.findItems(item => {
      if (item.properties.durability) {
        const durabilityPercent = item.properties.durability.currentDurability / item.properties.durability.maxDurability;
  ...
  getSpoiledItems(): InventorySlot[] {
    return this.inventory.findItems(item => {
      if (item.properties.perishable) {
        const freshnessPercent = item.properties.perishable.currentFreshness / item.properties.perishable.maxFreshness;
